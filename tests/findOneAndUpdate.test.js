...
modes.forEach((mode) => {
  describe(`SoftDelete - findOneAndUpdate (mode: ${mode})`, () => {
    let Model, Document
    ...
    it('Should not update deleted document', async () => {
      ...
      await Model.findOneAndUpdate({_id: Document._id}, {name: 'UpdatedDocument'})
    ...
    it('Should update deleted document when using withDeleted option', async () => {
      await Document.deleteOne()
      await Model.findOneAndUpdate({_id: Document._id}, {name: 'UpdatedDocument'}, {withDeleted: true})
      const Updated = await Model.findOne({_id: Document._id}, null, {withDeleted: true})
